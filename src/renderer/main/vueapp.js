    ...
    methods: {
        ...
        async refreshPlaylists(localOnly = false) {
            ...
            async function deepScan(parent = "p.playlistsroot") {
                ...
                const playlistData = await app.mk.api.music(`/v1/me/library/playlist-folders/${parent}/children/`)
                await asyncForEach(playlistData.data.data, async (playlist) => {
                    ...
                    try {
                        if (trackMap) {
                            let tracks = await app.mk.api.music(playlist.href + "/tracks").catch(e => {
                                // no tracks
                                e = null
                            })
        ...
        async getMadeForYou(attempt = 0) {
            ...
            try {
                let mfu = await app.mk.api.music("/v1/me/library/playlists?platform=auto&extend=editorialVideo&fields%5Bplaylists%5D=lastModifiedDate&filter%5Bfeatured%5D=made-for-you&include%5Blibrary-playlists%5D=catalog&fields%5Blibrary-playlists%5D=artwork%2Cname%2CplayParams%2CdateAdded")
